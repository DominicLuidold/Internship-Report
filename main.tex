...
The Better Life System has been in development since spring 2017 and is based on a multitier architecture as well as the PHP programming language and the PHP framework \enquote{Symfony}. Due to evolving project requirements and a related, historically grown complexity, the decision was made to successively replace the layer architecture by implementing the \enquote{Command Query Responsibility Segregation} pattern (short \enquote{CQRS}). The migration should promote maintainability and expandability as well as improve code quality and testability in the long term.
