...
Die Funktionsweise der von Symfony vorgeschlagenen Architektur kann, unter Bezugnahme auf die in Abschnitt~\ref{sub-sec:empfohlener-ansatz} beschriebenen Prinzipien, mit der Abbildung~\ref{fig:single-model-fowler} genauer beleuchtet werden:

Im Zuge der Entwicklung einer API wird oftmals auf CRUD beziehungsweise einen CRUD-ähnlichen Aufbau gesetzt. Die Nutzung eines einzelnen Models oder DTOs sowie der dazugehörenden Businesslogik fällt dabei für die vier Anwendungsfälle \textit{Create}, \textit{Read}, \textit{Update}, \textit{Delete} meistens relativ leicht. Auch bei weiterführenden Nutzungsfällen, bei denen beispielsweise mehrere Models zusammengeführt und entsprechend validiert werden müssen, wird die bestehende Logik hergenommen und keine spezifische Auftrennung von ausgeführten Aktionen vorgenommen. Dadurch entsteht eine Vermischung von lesenden und schreibenden Zugriffen in der Anwendungslogik und den Funktionalitäten des Models, was das System undurchsichtiger und stellenweise komplizierter macht. \parencite[Vgl.][]{fowler_cqrs_2011}

\vspace{0.5cm}

\begin{figure}[ht]
    \centering
    \includegraphics[scale=0.6]{images/fowler_single_model.png}
...
\textbf{Abfragen von Daten:} Ein Abfragen der Daten (ebenfalls durch das Frontend ausgelöst) führt dazu, dass diese von der Datenbank geladen und mittels des (in manchen Fällen auch mehrer) Query Models im entsprechenden Format serialisert werden. Das Query Model schließt die Aktion damit ab, dass die serialisierten Daten nach außen als Rückgabewert(e) weitergegeben werden. \parencite[Vgl.][]{heimeshoff_cqrs_2013}
