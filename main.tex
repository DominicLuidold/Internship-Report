...
\usepackage[
    ...
    pdftitle={Vergleich des CQRS Patterns mit der Herangehensweise einer gängigen Schichtenarchitektur}
...
Auf Wunsch der Firma Fusonic GmbH, im Auftrag der Viterma Handels GmbH, ist die vorliegende Arbeit bis zum 24. November 2023 für die öffentliche Nutzung zu sperren.
...
\noindent Dornbirn, am 24. November 2020\hfill Unterschrift des Verfassers
...
\begin{titlepage}
    ...
    \end{flushright}
    \begin{flushleft}
    \section*{Vergleich des CQRS Patterns mit der Herangehensweise einer gängigen Schichtenarchitektur}
    \vspace{1cm}
...
% Kurzreferat:
\newpage
\enlargethispage*{19pt}
\section*{Kurzreferat}
\label{sec:kurzreferat}

\subsection*{Vergleich des CQRS Patterns mit der Herangehensweise einer gängigen Schichtenarchitektur}

Das \enquote{Better Life System} ist eine mittels einer Weboberfläche und gängigen Webbrowsern bedienbare Anwendung. Handelsvertreterinnen rund um die Viterma Handels GmbH können diese nutzen, um ein Badezimmer anhand der persönlichen Anforderungen und Bedürfnisse der Endkunden zu konfigurieren und ein entsprechendes Angebot dafür zu erstellen.

\medskip

Das Better Life System ist bereits seit Frühjahr 2017 in Entwicklung und baut auf einer gängigen Schichtenarchitektur sowie der Programmiersprache PHP und dem PHP-Framework \enquote{Symfony} auf. Aufgrund von sich weiterentwickelnden Anforderungen an das Projekt und einer damit verbundenen, historisch gewachsenen Komplexität wurde der Entschluss getroffen, die Schichtenarchitektur sukzessive durch die Implementierung des \enquote{Command Query Responsibility Segregation} Patterns (\enquote{CQRS}) zu ersetzen. Die Umstellung soll die Wartbarkeit und Erweiterbarkeit fördern sowie die Codequalität und Testbarkeit nachhaltig verbessern.

\medskip

Das CQRS Pattern beschreibt die Aufteilung von lesenden und schreibenden Aktionen, die voneinander getrennt sind, unterschiedlich agieren sowie verschiedene Resultate zur Folge haben. Lesende Aktionen greifen lediglich auf Daten zu und liefern diese als Ergebnis zurück, während schreibende Aktionen Daten verändern und keinen Rückgabewert zur Folge haben. Durch die Nutzung von CQRS können komplexe und umfassende Domänen besser aufgeteilt und leichter gehandhabt werden. Die Implementierung stellt jedoch auch einen Mehraufwand dar, welcher nicht bei allen Anwendungsfällen im Verhältnis zum Endresultat steht.

\medskip

Die zu Beginn des Praktikums festgelegten Ziele konnten erfolgreich erfüllt werden und das Better Life System verwendet nun bei vorab bestehenden Teilbereichen und neu entwickelten Funktionalitäten das CQRS Pattern. Die angestrebte Verbesserung der Codequalität, leichtere Erweiterbarkeit sowie Testbarkeit konnte in den umgestellten Bereichen erzielt werden und die abschließende Umstellung erfolgt laufend in der weiteren Entwicklung des Projektes.
...
\subsection*{Comparison of the CQRS Pattern With the Approach of a Common Multi-Tier Architecture}

The \enquote{Better Life System} is an application that can be operated via a web interface and common web browsers. Sales representatives around Viterma Handels GmbH can use it to configure a bathroom according to the personal requirements and needs of the end customer and to create a corresponding offer for it.

\medskip

The Better Life System has been in development since spring 2017 and is based on a multi-tier architecture as well as the PHP programming language and the PHP framework \enquote{Symfony}. Due to evolving project requirements and a related, historically grown complexity, the decision was made to successively replace the multi-tier architecture by implementing the \enquote{Command Query Responsibility Segregation} pattern (\enquote{CQRS}). The migration should promote maintainability and expandability as well as improve code quality and testability in the long term.

\medskip

The CQRS pattern describes the separation of reading and writing actions that are separate, act differently and produce different results. Read actions only access and return data as a result, while write actions modify data and do not return a value. By using CQRS, complex and comprehensive domains can be better divided and are easier to handle. However, the implementation also represents an additional effort which is not always proportional to the final result in all use cases.

\medskip

The objectives set at the beginning of the internship were successfully met and the Better Life System now uses the CQRS pattern for pre-existing sub-areas and newly developed functionalities. The desired improvement of the code quality, easier expandability and testability was achieved in the refactored areas and the final migration is being carried out continuously in the further development of the project.
...
Das Better Life System - welches von Fusonic GmbH entwickelt wird und mittels einer Weboberfläche und gängigen Webbrowsern bedient werden kann - ermöglicht es Handelsvertreterinnen der Viterma Handels GmbH (beziehungsweise Vertreterinnen eines ihrer Franchise-Partner) zusammen mit ihren Endkunden ein Badezimmer anhand der jeweiligen Bedürfnisse auszusuchen und zu konfigurieren, um schlussendlich ein entsprechendes Angebot dafür erstellen zu lassen.

\section{Arbeitgeber Fusonic GmbH}
\label{sec:arbeitgeber}
Die Fusonic GmbH hat ihren Firmensitz in Götzis, Vorarlberg, und besteht aus einem Team von aktuell mehr als 25 Angestellten, Softwareentwicklerinnen und Projektleiterinnen. Diese sind aufgeteilt in diverse kleinere Teams, die intern unter anderem \enquote{Duck-Team} oder auch \enquote{Parrot-Team} genannt werden. Die Teams arbeiten dabei an jeweils eigenständigen Projekten und setzen diverse Technologien ein. Zu den verwendeten Technologien gehören unter anderem C\# mit .NET, PHP mit Symfony und JavaScript/TypeScript mit Angular \parencite[]["Übersicht aller Technologien"]{fusonic_gmbh_web_2020}. Während es regelmäßigen Austausch unter den Teams gibt, besteht jedes aus Frontend- sowie Backend-Entwicklerinnen, da die meisten Projekte - aufgrund der zugrundeliegenden Anwendungsfälle - aus einem Web-Frontend sowie serverseitigen Backend bestehen.
...
\begin{itemize}
    ...
    \item \textbf{View:} Im Falle der hier angenommenen Standardstruktur, wird der View-Teil mittels der Template Engine \textit{Twig} im gleichnamigen Verzeichnis abgehandelt. \parencite[][]{tutorials_point_i_pvt_ltd_symfony_nodate} Im Falle des BLS ist dies mittels Data Transfer Objects gelöst, die bei den jeweiligen API Endpunkten als serialisierte JSON-Response zurückgegeben werden.
...
Die Anwendung des CQRS Patterns im Better Life System hat mehrere Vorteile mit sich gebracht, die zum einen bereits in vorhergehenden Kapitel ausführlicher beleuchtet wurden (siehe beispielsweise Abschnitt~\ref{sub-sec:cqrs-vorteile} auf Seite \pageref{sub-sec:cqrs-vorteile}), zum anderen aber auch aus Sicht des Entwicklerteams positiv zu werten sind. Zwar bedarf es einer grundlegenden Konfiguration von Symfony Komponenten, damit das Pattern entsprechend angewendet werden kann, diese steht bei einem Projekt wie dem BLS jedoch im Verhältnis zum erzielten Endergebnis. Das Verständnis für das CQRS Pattern ist im Regelfall bereits nach kurzer Einarbeitung gegeben, wodurch zügig mit dem tatsächlichen Entwickeln begonnen werden kann.

Aufgrund der historisch gewachsenen Code-Basis und den sich stetig entwickelnden Anforderungen hat das BLS eine gewisse Komplexität erreicht. Die Nutzung einer gängigen Schichtenarchitektur war und ist zu jedem Zeitpunkt gut möglich, der Umstieg hat die Übersichtlichkeit und Zuständigkeit der einzelnen Komponenten jedoch merklich verbessert. Neben deutlich kleineren Klassen, einfacher testbaren Teilbereichen des Systems und einer verbesserten inhaltlichen Gruppierung ist ein leichterer Einstieg für neue Entwicklerinnen in das Projekt gegeben. Das Umsetzen neuer Funktionalitäten im Projekt fällt grundsätzlich leichter, birgt aufgrund der Anforderung der Rückwärtskompatibilität an machen Stellen jedoch auch ihre Tücken, wenn beide Architekturen in bereits bestehenden Komponenten zusammengeführt werden müssen.

Rückblickend kann zusammengefasst werden, dass die bestehenden - sowie die während der Umstellung neu hinzugekommenen - Erwartungen an das CQRS Pattern erfüllt werden konnten. Die im Verlauf des Berufspraktikums erzielten Erfolge und Verbesserungen des Systems rechtfertigen den erhöhten Aufwand, der durch die Umstellung und die etwaig benötigte Konfiguration entstanden ist, und spiegeln sich spürbar positiv im Backend des Better Life System wider.

\section{Ausblick}
\label{sec:ausblick}
Das Better Life System befindet sich zum Zeitpunkt des Verfassens dieser Arbeit weiterhin in Entwicklung und wird voraussichtlich auch noch einige Zeit weiterentwickelt werden. Da die Umstellung auf das CQRS Pattern laufend durchgeführt wurde und noch nicht vollständig abgeschlossen ist, müssen noch weitere Teile des bestehenden Systems aktualisiert und entsprechend angepasst werden. Die Basis wurde dafür während des Berufspraktikums geschaffen sowie projektspezifische Herangehensweisen und Standards festgelegt, auf denen weitere Funktionalitäten und Anpassungen aufgebaut werden können.

\medskip

Die Nutzung des CQRS Patterns hat sich, im Vergleich zu einer gängigen Schichtenarchitektur, bereits zum Zeitpunkt des Berufspraktikums und auch innerhalb der Fusonic GmbH bewährt und lässt sich somit auch für andere Projekte, unter Berücksichtigung der allgemeinen Vor- beziehungsweise Nachteile, empfehlen.
...
\vspace{5cm}
\noindent
Dornbirn, am 24. November 2020\hfill Dominic Luidold
